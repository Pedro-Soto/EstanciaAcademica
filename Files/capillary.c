...
/* XWALL_BOTTOM sets the wetting properties only on the bottom wall. The top one gets H=C=0.*/
...
// In gradient_3d_27pt_solid.c, the h map is written in the form of an array c = wet[0]; h = wet[1];
...
int main(int argc, char ** argv) {
 ...
 char  wall_name_file[800];
  ...
  double * map_h;
  double * map_c; // To account for both wetting coefficients C an H
  ...
  case YWALL:
	...
	for (k = 0; k < zmax; k++) {
	  ...
	  map_in[n] = MAP_FLUID;
	  map_sig[n] = 0.0;
	  // Calcula el tamaño maximo de la tuberia
	  double b = I;
...
////////////////////////////////////// Export the wall profile


if (output_type == STATUS_WITH_H)  {
  sprintf(wall_name_file,"Wall_config.dat");    
  Wall_file=fopen(wall_name_file,"w");
  fprintf(Wall_file,"#x y z n map H\n");
	...
	if (map_in[n] == MAP_BOUNDARY) { fprintf(Wall_file,"%i %i %i %i %d %f\n", i, j, k, n, MAP_BOUNDARY, map_h[n]); }
	if (map_in[n] == MAP_FLUID)    { fprintf(Wall_file,"%i %i %i %i %d %f\n", i, j, k, n, MAP_FLUID, map_h[n]); }
...
if (output_type == STATUS_WITH_C_H)  {
  sprintf(wall_name_file,"Wall_config.dat");    
  Wall_file=fopen(wall_name_file,"w");
  fprintf(Wall_file,"#x y z n map H C\n");
	...
	if (map_in[n] == MAP_BOUNDARY) { fprintf(Wall_file,"%i %i %i %i %d %f %f\n", i, j, k, n, MAP_BOUNDARY, map_h[n], map_c[n]); }
	if (map_in[n] == MAP_FLUID)    { fprintf(Wall_file,"%i %i %i %i %d %f %f\n", i, j, k, n, MAP_FLUID, map_h[n], map_c[n]); }
