...
/*********************************************************************************************************************************************/
	...
	printf("#### Drop Low viscosity\nLooking for initial position: ");

	int zinitial=1;
	k=1;
	
	double Triple1, Triple2, Tip1, Tip2;
	Triple1=0;
	Triple2=0;
	Tip1=0;
	Tip2=0;
	...
	for (k = 1; k < NZ; k++)
		{
		if ( phi[icentre][1][k-1]<0.5 && phi[icentre][1][k]>=0.5 ) { Triple1=k; };
		if ( phi[icentre][1][k-1]>0.5 && phi[icentre][1][k]<=0.5 ) { Triple2=k; };
		if ( phi[icentre][jcentre][k-1]<0.5 && phi[icentre][jcentre][k]>=0.5 ) { Tip1=k; };
		if ( phi[icentre][jcentre][k-1]>0.5 && phi[icentre][jcentre][k]<=0.5 ) { Tip2=k; };		
		}



	sprintf(file,"Velocities.dat");
	stat_file=fopen(file,"r");
	if (stat_file == NULL)
		{	
		stat_file=fopen(file,"w");
		fprintf(stat_file, "#1-time 2-Triple1 3-Tip1 4-Triple2 5-Tip2 6-length1 7-length2\n");
		}
		fclose(stat_file);

	stat_file=fopen(file,"a");
	fprintf(stat_file, "%0.1f %0.3f %0.3f %0.3f %0.3f %0.3f %0.3f\n", time, Triple1, Tip1, Triple2, Tip2, (Tip1-Triple1), (Tip2-Triple2) );
	fclose(stat_file);
