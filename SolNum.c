...
int main () {
    ...
    // Agregar una forma de leer b_., eta1, eta2, L, delta,  de canal de capillary.c o de input 
    ...
    float R = (12.0/(b0*b0))*(eta1+eta2)*(L/2.0);

    // Cálculo de Flujo (Q)
    float Q = (deltaP/R);
    float z1_inicial = 0;
    float z1 = (Q*delta_t) / (b0-2*amplitud*sin((2 * pi * z) / L)); 
